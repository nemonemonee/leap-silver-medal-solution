...
src_scl = torch.stack([src[:, i].repeat(1, 3) for i in input_scl_idx], dim=-1)
...
train_loader = DataLoader(train_dataset, batch_size=4096, num_workers=8, shuffle=True)
val_loader = DataLoader(val_dataset, batch_size=4096, num_workers=8, shuffle=False)

torch.set_float32_matmul_precision('high')
new_mask = torch.logical_and(mask, label_std > 1.1e-8)
model = UNet1D(mask=new_mask, input_dim=input_dim, scalar_dim=scalar_dim, output_dim=14)

checkpoint_callback = ModelCheckpoint(
    dirpath='../ckpt/',
    filename='unetv2-{epoch:02d}-{val_loss:.2f}',
    save_top_k=3,
    monitor='val_loss',
    mode='min'
)
